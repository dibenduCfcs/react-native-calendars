...
import React, { Fragment, useCallback, useMemo, forwardRef, useImperativeHandle, useRef, useEffect } from 'react';
...
const CalendarHeader = forwardRef((props, ref) => {
    ...
    useEffect(()=>{
        _renderHeader()
    },[month,onPressArrowLeft,onPressArrowRight])
    ...
    const _renderHeader = () => {
        ...
        if (renderHeader) {
            return renderHeader(month,onPressLeft,onPressRight);
        }
    ...
    return  showCalender?(<View testID={testID} style={propsStyle} accessible accessibilityRole={'adjustable'} accessibilityActions={accessibilityActions} onAccessibilityAction={onAccessibilityAction} accessibilityElementsHidden={accessibilityElementsHidden} // iOS
